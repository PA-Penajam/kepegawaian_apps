...
### Scope: When to Use and When NOT to Use Context7

**ALWAYS use Context7** to fetch current documentation whenever the task involves a library, framework, SDK, API, CLI tool, or cloud service — even well-known ones like React, Next.js, Prisma, Express, Tailwind, Django, or Spring Boot. This includes:

- API syntax and method signatures
...
**Use Context7 even when you think you already know the answer** — your training data may not reflect recent changes. Prefer Context7 over web search for library documentation.

**Do NOT use Context7 for:**

- Refactoring existing code (no library lookup needed)
- Writing scripts from scratch that don't depend on external libraries
- Debugging business logic (not library-related bugs)
- Code review (unless reviewing library usage patterns)
- General programming concepts (algorithms, data structures, design patterns)

### Context7 Tool Usage Steps (MANDATORY)

Follow these exact steps every time you use Context7:

```
Step 1: resolve-library-id
   - Use the library name AND the user's question as input
   - Exception: skip this step if the user provides an exact library ID in /org/project format

Step 2: Pick the Best Match
   The resolve step may return multiple results. Select the best one by evaluating:
   - Exact name match with the library being used
   - Description relevance to the user's question
   - Code snippet count (more = better coverage)
   - Source reputation (High or Medium preferred)
   - Benchmark score (higher is better)
   
   If results don't look right:
   - Try alternate names (e.g., "next.js" instead of "nextjs")
   - Rephrase the question
   - Use version-specific IDs when the user mentions a specific version

Step 3: query-docs
   - Use the selected library ID (format: /org/project)
   - Pass the user's FULL question as the query (not single words or fragments)

Step 4: Answer Using Fetched Docs
   - Base your response on the documentation returned by Context7
   - If Context7 results conflict with your internal knowledge, ALWAYS prioritize Context7
...
```
User: "Bagaimana cara setup middleware di Next.js 15 App Router?"

Agent internal steps:
1. resolve-library-id("next.js", "middleware setup in App Router")
2. Pick best match → /vercel/next.js (High reputation, highest snippet count)
3. query-docs("/vercel/next.js", "How to setup middleware in Next.js 15 App Router")
4. Answer in Bahasa Indonesia based on the fetched documentation
```
